...
class Battle(commands.GroupCog):
    ...
    async def add(
        ...
        if await countryball.is_locked():
            await interaction.followup.send(
                f"This {settings.collectible_name} is currently in an active battle, trade "
                "or donation, please try again later.",
                ephemeral=True,
            )
            return

        battler.proposal.append(countryball)
        await interaction.followup.send("Asset added.", ephemeral=True)
    ...
    async def remove(self, interaction: discord.Interaction, countryball: BallInstanceTransform):
        ...
        await interaction.response.send_message("Asset removed.", ephemeral=True)
