...
class BattleView(View):
    ...
    async def lock(self, interaction: discord.Interaction, button: Button):
        ...
        if not battler.proposal:
            await interaction.response.send_message(
                "You cannot have an empty proposal.", ephemeral=True
            )
            return
        if battler.locked:
            await interaction.response.send_message(
...
class BattleMenu:
    ...
    def _generate_embed(self):
        ...
        self.embed.description = (
            f"Add or remove {settings.plural_collectible_name} you want to propose to the other "
            f"player using the {add_command} and {remove_command} commands.\n"
    ...
    async def confirm(self, battler: BattlingUser, interaction: discord.Interaction) -> bool:
        ...
        for countryball in self.battler1.proposal:
            attack1 += countryball.attack
            health1 += countryball.health

        for countryball in self.battler2.proposal:
            attack2 += countryball.attack
            health2 += countryball.health

        worl1 = attack1 / health2 if health2 > 0 else 0
        worl2 = attack2 / health1 if health1 > 0 else 0
        ...
        if self.battler1.accepted and self.battler2.accepted:
            ...
            if winner is None:
                self.embed.description = (
                    "**This battle has ended in a draw, no winner could be decided. "
                    "Use stronger countryballs next time!"
                )
                self.battler1.player.battles_drawn += 1
                self.battler2.player.battles_drawn += 1
                await self.battler1.player.save()
                await self.battler2.player.save()
            else:
                self.embed.description = (
                    f"**The battle has concluded! {winner.user.name} is the winner!**"
                )
                winner.player.battles_won += 1
                await winner.player.save()
                if winner == self.battler1:
                    self.battler2.player.battles_lost += 1
                    await self.battler2.player.save()
                else:
                    self.battler1.player.battles_lost += 1
                    await self.battler1.player.save()
